...
class ContrainteBinaire(Contrainte):
    ...
    def propage(self, var):
        """ Propage l'assignation d'une variable au label de la seconde.
            ...
            :param var: la variable fixee
            :return: True si la contrainte est toujours satisfiable avec la valeur fixee
        """
        ...
        for l in variable.label[:]:
            if not self.est_valide(variable, l):
                variable.label.remove(l)

        # si il existe au moins une valeur possible
        # l'assignation est consistante
